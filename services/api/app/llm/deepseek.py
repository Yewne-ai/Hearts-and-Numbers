...
def _system_prompt_with_mode(
    ...
    [2026-08-18] chat_mode 是用户在深聊页自己选的（文档 4.4）。它比推断结果
    优先，所以**即使 mode 为 None 也要往下走** —— 分类器关掉或判失败时，
    用户选的模式仍然必须生效。优先级判断在 compose_system_prompt 里，
    这里不重复实现。
    """
    base = _system_prompt_for(persona)
    if mode is None and chat_mode is None:
        return base
    return compose_system_prompt(base, persona, mode, chat_mode=chat_mode)
...
class DeepSeekProvider:
    async def complete(
        ...
        messages: list[dict] = [
            {
                "role": "system",
                "content": _system_prompt_with_mode(persona, mode, chat_mode),
            }
        ]
        if history:
